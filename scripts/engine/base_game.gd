...
func _ready() -> void:
	...
	Core.audio.set_volume(Core.AudioType.MUSIC, 0.5)
	Core.audio.set_volume(Core.AudioType.SFX, 0.5)
	Core.audio.set_volume(Core.AudioType.AMBIENCE, 0.5)
...
func reset(reset_type_: Core.ResetType) -> void:
	...
	if reset_type_ == Core.ResetType.START:
		...
		await change_level_mode(Core.START_LEVEL, Core.LevelMode.GAME)
...
func menu() -> void:
	...
	await change_level_mode(Core.MENU_LEVEL, Core.LevelMode.MENU)
