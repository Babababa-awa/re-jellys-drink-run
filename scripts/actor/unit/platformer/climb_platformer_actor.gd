...
func move_process(_delta: float) -> void:
	...
	var direction_y = sign(unit.actions.get_axis(action_climb_up, action_climb_down, true))
	var direction_x = sign(unit.actions.get_axis(action_climb_left, action_climb_right, true))
	...
	if unit.unit_mode == Core.UnitMode.CLIMBING or climb_off_behavior == Core.PlatformerBehavior.NONE:
		if is_in_climb_down_area:
			if not is_in_climb_left_area and direction_x < 0.0:
				direction_x = 0.0
			elif not is_in_climb_right_area and direction_x > 0.0:
				direction_x = 0.0
	
	var velocity_: Vector2 = Vector2.ZERO
	
	if climb_speed == Core.UnitSpeed.SLOW:
		velocity_.x = direction_x * slow_climbing_speed
		velocity_.y = direction_y * slow_climbing_speed
	elif climb_speed == Core.UnitSpeed.FAST:
		velocity_.x = direction_x * fast_climbing_speed
		velocity_.y = direction_y * fast_climbing_speed
	else:
		velocity_.x = direction_x * normal_climbing_speed
		velocity_.y = direction_y * normal_climbing_speed
...
func _action_climb_up() -> void:
	#print(_cancel_action_climb_up)
	#print(is_in_climb_area)
	#print(is_in_climb_down_area)
	#print(is_in_climb_up_area)
