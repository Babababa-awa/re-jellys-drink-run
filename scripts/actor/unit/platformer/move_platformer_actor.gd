extends UnitActor
class_name MovePlatformerActor
...
var action_move_left: StringName = &"player_move_left"
var action_move_right: StringName = &"player_move_right"
var action_move_up: StringName = &"player_move_up"
var action_move_down: StringName = &"player_move_down"
var action_move_slow: StringName = &"player_move_slow"
var action_move_fast: StringName = &"player_move_fast"
...
func physics_process(delta_: float) -> void:
	...
	_internal_crouch_direction.x = 0
...
func move_process(_delta: float) -> void:
	...
	if can_unit_input():
		direction_x = sign(unit.actions.get_axis(action_move_left, action_move_right, true))
		#direction_y = unit.get_axis(action_move_up, action_move_down)
	...
	if move_speed == Core.UnitSpeed.SLOW:
		velocity_.x = direction_x * slow_move_speed
	elif move_speed == Core.UnitSpeed.FAST:
		velocity_.x = direction_x * fast_move_speed
	else:
		velocity_.x = direction_x * normal_move_speed
	...
	if crouch_actor != null and crouch_actor.is_crouching:	
		if fall_actor == null or not fall_actor.is_in_air:
			if move_crouch_behavior == Core.PlatformerBehavior.NONE:
				_internal_crouch_direction.x = direction_x
...
func _update_unit_direction() -> void:
	...
	if (previous_unit_direction_x != unit.unit_direction_x or 
		...
		_update_state(Core.ActorState.UPDATE, &"unit_direction")
...
func _update_unit_movement() -> void:
	...
	if previous_unit_movement != unit.unit_movement:
		_update_state(Core.ActorState.UPDATE, &"unit_movement")
...
func _update_unit_speed() -> void:
	...
	if previous_unit_speed != unit.unit_speed:
		_update_state(Core.ActorState.UPDATE, &"unit_speed")
...
func _update_unit_stance() -> void:
	...
	if previous_unit_stance != unit.unit_stance:
		_update_state(Core.ActorState.UPDATE, &"unit_stance")
...
func set_locked_unit_direction_x(value_: Core.UnitDirection) -> void:
	locked_direction_x = value_
	
func set_locked_unit_direction_y(value_: Core.UnitDirection) -> void:
	locked_direction_y = value_
	
func lock_unit_direction_x() -> void:
	if unit.unit_direction_x != Core.UnitDirection.NONE:
		set_locked_unit_direction_x(unit.unit_direction_x)
	else:
		set_locked_unit_direction_x(unit.previous_unit_direction_x)
		
func unlock_unit_direction_x() -> void:
	set_locked_unit_direction_x(Core.UnitDirection.NONE)
